...
class GXAVPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        scene = context.scene
        row = layout.row()
        row.operator("object.gx_create_base", icon="PROPERTIES", text="(re)Create Visualizer Base")


        try:
            if bpy.context.scene['gx_init'] == 1:
                layout.label(text="Parameters:", icon="PROPERTIES")
                row = layout.row()
                ...
                row.prop(scene, "gx_slash_rotate", icon="PROPERTIES")
                ...
                if bpy.context.scene['gx_custom_object_b'] == True:
                    ...
                    row.operator("object.gx_create_base", icon="PROPERTIES", text="Update")

                try:
                    if bpy.context.scene['gx_type'] == 0:
                        row = layout.row()
                        row.prop(scene, "gx_type", icon="PROPERTIES", text="Mode")
                        split = layout.split()
                        col = split.column()
                        col.label(text="Object Scale:", icon="PROPERTIES")
                        col = split.column(align=True)
                        col.prop(scene, 'gx_scale_x', text="X")
                        col.prop(scene, 'gx_scale_y', text="Y")
                        col.prop(scene, 'gx_scale_z', text="Z")
                        row = layout.row()
                        row.label(text="Array modifier space:", icon="PROPERTIES")
                        row.prop(scene, "gx_space_array")
                        row = layout.row()
                        row.label(text="Merge Array", icon="PROPERTIES")
                ...
                row = layout.row()
                box = layout.box()
                box.label(text="Channels to bake and audio files path:", icon="PROPERTIES")
                if bpy.context.scene['gx_channels'] == 0:
                    box.prop(scene, "gx_channels", icon="PROPERTIES")
                ...
                row = layout.row()
                row.prop(scene, "gx_mode", icon="PROPERTIES")
                row = layout.row()
                ...
                row = layout.row()
                row.operator("object.gx_bake", icon="PROPERTIES", text="(re)Bake animation data")
                row = layout.row()
                row.label(text="")

                box = layout.box()
                box.label(text="'Bake Sound to F-Curves' options:", icon="PROPERTIES")
                split = box.split()
                col = split.column(align=True)
                ...
                row = layout.row()
                row.operator("object.gx_init_variables", icon="PROPERTIES", text="Init/Reset Variables")
        except:
            False
