...
if "bpy" in locals():
    import importlib
    importlib.reload(make_stepshape)
else:
...
def insert_keyframe_curve(curve, frame, value, interpolation):
    """ Insert a keyframe in a curve """
...
def bone_to_object(transform, obj, bone, location=False):
    """
    convert bone local transform into object coordinate
    """
    data_bone = obj.data.bones[bone]
    return bone_rest_mat(data_bone, location) * transform
...
def object_to_bone_generic(transform, obj, bone, is_location):
    '''
    Place a object space transformation into bone space, checks location
    '''
    data_bone = obj.data.bones[bone]
    return bone_rest_mat(data_bone, is_location).inverted() * transform
...
def framepreserve(fn):
    ...
    def wrapped(*args, **kwargs):
        scene = bpy.context.scene # is it bad to get context here?
