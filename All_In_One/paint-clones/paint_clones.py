...
def generateRotationMatrix(direction, guide, trackAxis="Z", guideAxis="X"):
    ...
    if y == zero:
        if guideAxis == "X":
            if z.cross(xAxis) != zero:
                y = z.cross(xAxis)
            else:
                y = zAxis
        elif guideAxis == "Y":
            if z.cross(yAxis) != zero:
                y = z.cross(yAxis)
            else:
                y = zAxis
        elif guideAxis == "Z":
            if z.cross(zAxis) != zero:
                y = z.cross(zAxis)
            else:
                y = yAxis
    ...
    def execute(self, context):
        ...
        if len(context.selected_objects) > 0:
            ...
            if pct.track_axis[-1:] == 'Z':
                x, y = 0, 1
            sizes = [(obj.dimensions[x] * obj.scale[x] + obj.dimensions[y] * obj.scale[y]) / 2.0 for obj in
...
def copy_modifier(src_modifier, dst_object):
    new_modifier = dst_object.modifiers.get(src_modifier.name, None)
    if not new_modifier:
        new_modifier = dst_object.modifiers.new(src_modifier.name, src_modifier.type)

    # collect names of writable properties
    properties = [p.identifier for p in src_modifier.bl_rna.properties
                  if not p.is_readonly]

    # copy those properties
    for prop in properties:
        setattr(new_modifier, prop, getattr(src_modifier, prop))


def copy_settings_clones(pct, newObj, oldObj):
    # Copy Parent
    if oldObj.parent is not None:
        newObj.parent = oldObj.parent
        newObj.matrix_parent_inverse = oldObj.parent.matrix_world.inverted()
    ...
    for old_modifier in oldObj.modifiers.values():
        copy_modifier(old_modifier, newObj)
...
def pick_and_clone(self, context, event, ray_max=10000.0):
    ...
    if best_obj is not None:
        ...
        bpy.ops.transform.rotate(value=randRotX * pct.randRotationX, axis=get_obj_axis(newDup, 'X'),
                                 proportional='DISABLED')
        bpy.ops.transform.rotate(value=randRotY * pct.randRotationY, axis=get_obj_axis(newDup, 'Y'),
                                 proportional='DISABLED')
        bpy.ops.transform.rotate(value=randRotZ * pct.randRotationZ, axis=get_obj_axis(newDup, 'Z'),
                                 proportional='DISABLED')
