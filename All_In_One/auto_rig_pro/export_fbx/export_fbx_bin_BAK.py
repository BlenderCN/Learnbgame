...
def fbx_animations_do(scene_data, ref_id, f_start, f_end, start_zero, objects=None, force_keep=False):
	...
	while currframe <= f_end:	
		...
		for ob_obj, (anim_loc, anim_rot, anim_scale) in animdata_ob.items():
			...
			#if ob_obj.name == "thigh_stretch.r":
			#	rot_degree = [math.degrees(rot[0]), math.degrees(rot[1]), math.degrees(rot[2])]
			#	print(rot_degree)
			p_rots[ob_obj] = rot
			...
			for act in bpy.data.actions:
				...
				if scn.arp_export_rig_type == "humanoid" or scn.arp_export_rig_type == "mped":
					...
					if scn.arp_bake_actions:
						if "arp_export_h_actions" in scn.keys():
							export_baked_actions = scn.arp_export_h_actions
							
						if export_baked_actions:
						
							if (act.name[:2] == "h_" or act.name[:3] == "mp_") and (scn.arp_export_rig_type == "humanoid" or scn.arp_export_rig_type == "mped"):
								save_action = True
						...
						if "arp_export_name_actions" in scn.keys():
							if scn.arp_export_name_actions:
								if scn.arp_export_name_string not in act.name:
									save_action = False
								
					
				# if Generic type
				if scn.arp_export_rig_type == "generic":
					if scn.arp_bake_actions:
						save_action = True
