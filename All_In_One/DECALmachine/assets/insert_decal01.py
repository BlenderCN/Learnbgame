...
class InsertDecal01(bpy.types.Operator):
    bl_idname = "machin3.insert_decal01"
    bl_label = "Insert Decal01"

    decal01_name = StringProperty()

    def execute(self, context):
        insert_decal01(self.decal01_name)
        context.area.tag_redraw()
        return {"FINISHED"}


def insert_decal01(decal01_name):
    ...
    try:
        decal = load_object_from_other_file(filepath, decal01_name)
        bpy.context.scene.objects.link(decal)
    except NameError:
        open_error_message("The Decal '{}' does not exist in the assets file".format(decal01_name))
        return
    except OSError:
