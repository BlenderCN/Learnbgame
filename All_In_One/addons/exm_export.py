...
bl_info = {
    ...
    "warning": "",
    "wiki_url": "",
    "tracker_url": "https://github.com/excessive/iqm/issues",
    "category": "Learnbgame",
    }
...
class IQMFile:
    ...
    def addExtras(self, exm_meta):
        self.meta_json += bytes(exm_meta, encoding="utf8") + b'\x00'
    ...
    def export(self, file, usebbox = True):
        ...
        if self.meta_json:
            ofs_meta = self.filesize
            while len(self.meta_json) % 4:
                self.meta_json += b'\x00'
            self.filesize += len(self.meta_json)
            num_meta = 1
        else:
            ofs_meta = 0
            num_meta = 0

        file.write(IQM_HEADER.pack('INTERQUAKEMODEL'.encode('ascii'), 2, self.filesize, 0, len(self.textdata), ofs_text, len(self.meshdata), ofs_meshes, num_vertexarrays, self.numverts, ofs_vertexarrays, self.numtris, ofs_triangles, ofs_neighbors, len(self.jointdata), ofs_joints, len(self.posedata), ofs_poses, len(self.animdata), ofs_anims, self.numframes, self.framesize, ofs_frames, ofs_bounds, num_meta, ofs_meta, 0, 0))
        ...
        file.write(self.meta_json)

def findArmature(context, selected_only = False):
    armature = None
    scene = context.view_layer
    objects = [obj for obj in scene.objects if not obj.hide_viewport]
    if selected_only:
            ...
            for tri in data.loop_triangles:
                ...
                for loop_index in tri.loops:
                    ...
                    if not tri.use_smooth:
                        vertno = mathutils.Vector(tri.normal)
                    else:
                    ...
                    if colors:
                        ...
                        vertcol = (255, 255, 255, 255)

                    if alpha:
                        vertalpha = alpha[loop_index].color
                        if vertcol:
...
def exportIQM(context, filename, usemesh = True, usemodifiers = True, useskel = True, usebbox = True, usecol = False, meshopt = True, matfun = (lambda prefix, image: image), derigify = False, boneorder = None, flipyz = False, reversewinding = True, exm_meta = None, selected_only = False):
    ...
    if filename.lower().endswith('.exm'):
        filetype = 'EXM'
    elif filename.lower().endswith('.iqm'):
        filetype = 'IQM'
    else:
    ...
    if filename:
        try:
            ...
            return
        if filetype == 'EXM' or filetype == 'IQM':
...
def getJSON(context,
    ...
    if selected_only:
        objects = scene.selected_objects
    ...
    for o in objects_curve:
        if not hasattr(o.data.splines.active, 'type'):
            print('skipping object %s (no type attribute)' % o.name)
            continue
        ...
        objdata = {
            ...
            # FIXME: not all properties are serializable!
...
class ExportEXM(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def check(self, context):
        ...
        filepathalt = bpy.path.ensure_ext(self.filepath, '.exm')
...
def menu_func(self, context):
    self.layout.operator(ExportEXM.bl_idname, text="Excessive Model (.exm, .iqm)")

def register():
    bpy.utils.register_class(ExportEXM)
    bpy.types.TOPBAR_MT_file_export.append(menu_func)

def unregister():
    bpy.utils.unregister_class(ExportEXM)
