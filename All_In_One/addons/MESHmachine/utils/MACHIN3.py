...
def apply_all_mods(obj, settings='PREVIEW'):
    if obj.type == "MESH":
        scn = bpy.context.scene
        old_mesh = obj.data
        new_mesh = obj.to_mesh(scn, True, settings)
        obj.modifiers.clear()
        obj.data = new_mesh
...
def set_timestamp(obj, timestamp=None, silent=True):
    if timestamp is None:
        import time
        timestamp = time.time()
    obj["timestamp"] = timestamp
    # obj.DM.timestamp = timestamp

    if not silent:
        print("Set '%s' timestamp to '%f'." % (obj.name, timestamp))
    return timestamp
...
def DM_prefs():
    ...
    return bpy.context.user_preferences.addons[os.path.basename(os.path.dirname(os.path.dirname(__file__)))].preferences
...
def debug_idx(obj=None):
    if obj:
        bpy.app.debug = True
        obj.mesh.show_extra_indices = True
    else:
        bpy.context.scene.display_indices = -1
        bpy.ops.view3d.index_visualiser('INVOKE_DEFAULT')
...
def delete_all(keepobjs=None, keepmats=None, debug=False):
    for obj in bpy.data.objects:
        if keepobjs:
            if obj in keepobjs:
                continue
    ...
    for mat in bpy.data.materials:
        ...
        if debug:
            print(" » Removing material %s" % mat.name)
    ...
    for group in bpy.data.groups:
        if debug:
            print(" » Removing group %s" % group.name)

        bpy.data.groups.remove(group, do_unlink=True)

    for group in bpy.data.node_groups:
        if debug:
            print(" » Removing node group %s" % group.name)
