...
BAKED_SUFFIX = "_B4A_BAKED"

class B4A_Anim_Baker(bpy.types.Operator):
    '''Bake animation for selected armature object'''

    bl_idname = "b4a.animation_bake"
    bl_label = "B4A Animation Bake"
...
class B4A_Constraints_Muter(bpy.types.Operator):
    '''Mute bone constraints for selected armature object'''

    bl_idname = "b4a.constraints_mute"
    bl_label = "B4A Constraints Mute"
...
class B4A_Constraints_UnMuter(bpy.types.Operator):
    '''Unmute bone constraints for selected armature object'''

    bl_idname = "b4a.constraints_unmute"
    bl_label = "B4A Constraints UnMute"
...
class B4A_AnimBakerPanel(bpy.types.Panel):
    bl_label = "B4A Anim Baker"
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.prop(obj, "b4a_anim_clean_keys", text="Clean keyframes")

        row = layout.row()
        row.template_list("UI_UL_list", "OBJECT_UL_anim_baker",
                obj, "b4a_anim", obj, "b4a_anim_index", rows=3)
        col = row.column(align=True)
        col.operator("b4a.anim_add", icon='ZOOMIN', text="")
        col.operator("b4a.anim_remove", icon='ZOOMOUT', text="")

        anim = obj.b4a_anim
        if anim:
            anim_index = obj.b4a_anim_index
        ...
        row = layout.row()
        row.operator("b4a.animation_bake", text="Bake", icon="REC")

        row = layout.row(align=True)
        row.operator("b4a.constraints_mute", text="Cons Mute")
        row.operator("b4a.constraints_unmute", text="Cons Unmute")

class B4A_Anim(bpy.types.PropertyGroup):
    # property name is already here
    pass

class B4A_AnimAddOperator(bpy.types.Operator):
    bl_idname      = 'b4a.anim_add'
...
class B4A_AnimRemOperator(bpy.types.Operator):
    bl_idname      = 'b4a.anim_remove'
    ...
    def invoke(self, context, event):
        ...
        if obj.b4a_anim_index >= 0:
           anim.remove(obj.b4a_anim_index)
           obj.b4a_anim_index -= 1

        return {'FINISHED'}

def register():
    bpy.utils.register_class(B4A_Anim_Baker)
    bpy.utils.register_class(B4A_Constraints_Muter)
    bpy.utils.register_class(B4A_Constraints_UnMuter)
    bpy.utils.register_class(B4A_AnimBakerPanel)
    bpy.utils.register_class(B4A_AnimAddOperator)
    bpy.utils.register_class(B4A_AnimRemOperator)

    bpy.utils.register_class(B4A_Anim)

    bpy.types.Object.b4a_anim =\
            bpy.props.CollectionProperty(type=B4A_Anim, name="B4A: animation")
    bpy.types.Object.b4a_anim_index =\
            bpy.props.IntProperty(name="B4A: animation index")

def unregister():
    bpy.utils.unregister_class(B4A_Anim_Baker)
    bpy.utils.unregister_class(B4A_Constraints_Muter)
    bpy.utils.unregister_class(B4A_Constraints_UnMuter)
    bpy.utils.unregister_class(B4A_AnimBakerPanel)
    bpy.utils.unregister_class(B4A_AnimAddOperator)
    bpy.utils.unregister_class(B4A_AnimRemOperator)

    bpy.utils.unregister_class(B4A_Anim)
