...
bl_info = {
	'name': 'Joe/JPK tools 3.0',
	'description': 'Import-Export to FirstDrive track files',
...
class joe_pack:
		...
		for obj in objlist:
			...
			if obj.data.tessface_uv_textures[0].data[0].image:
				...
				for mat_slot in obj.material_slots:
					for mtex_slot in mat_slot.material.node_tree.nodes:
						if hasattr(mtex_slot, 'image'):
							image =  mtex_slot.image
							break
						if image: break
					if image: break
			...
			#if obj.material_slots[0].material.node_tree.nodes[2].image:
			#	image = obj.material_slots[0].material.node_tree.nodes[2].image.name
			if not image:
				print(obj.name + ' not exported. No texture linked.')
				continue
			#else: print(obj.name + ' will be SUCCESSFULLY exported!')
	...
	def load(self, filename):
		...
		# header
		version = 3#file.read(len(joe_pack.version))
...
class trackobject:
	...
	def to_obj(self, object):
		...
		if self.values[8] == '1': trackobject.grp['collidable'].link(object)
		if self.values[9] == '1': trackobject.grp['collidable'].link(object)
...
class export_joe(bpy.types.Operator, ExportHelper):
	...
	def execute(self, context):
		...
		if len(bpy.context.selected_objects[:]) < 1:
			raise NameError('Please select at least one object!')
		object = self.object
		...
		for obj in bpy.context.selected_objects:
			...
			try:
				...
				self.report({'INFO'},  obj.name + ' exported')
		return {'FINISHED'}
...
def menu_export_joe(self, context):
	self.layout.operator(export_joe.bl_idname, text = 'VDrift3 JOE (.joe)')


def menu_import_joe(self, context):
	self.layout.operator(import_joe.bl_idname, text = 'VDrift3 JOE (.joe)')


def menu_import_image(self, context):
	self.layout.operator(import_image.bl_idname, text = 'VDrift3 Texture (.png)')


def menu_export_jpk(self, context):
	self.layout.operator(export_jpk.bl_idname, text = 'VDrift3 JPK (.jpk)')


def menu_import_jpk(self, context):
	self.layout.operator(import_jpk.bl_idname, text = 'VDrift3 JPK (.jpk)')


def menu_import_joe_list(self, context):
	self.layout.operator(import_joe_list.bl_idname, text = 'VDrift3 Track Objects (list.txt)')


def menu_export_trk(self, context):
	self.layout.operator(export_trk.bl_idname, text = 'VDrift3 Roads (.trk)')


def menu_import_trk(self, context):
	self.layout.operator(import_trk.bl_idname, text = 'VDrift3 Roads (.trk)')


def menu_export_track(self, context):
	self.layout.operator(export_track.bl_idname, text = 'VDrift3 Track Info (track.txt)')


def menu_import_track(self, context):
	self.layout.operator(import_track.bl_idname, text = 'VDrift3 Track Info (track.txt)')
