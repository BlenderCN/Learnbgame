...
from .utils import *
from .operator import UvPackOperator, UvOverlapCheckOperator, UvMeasureAreaOperator, UvValidateOperator
...
class UvPackmasterPreferences(AddonPreferences):
    ...
    grouped_pack = BoolProperty(
                    name='Group By Material (Experimental)',
                    description="Make sure islands sharing the same material are packed together. This feature is experimental: especially it can block when used together with 'Pack To Others' option in case no appropriate place is found for an island in the unit UV square. In such situation simply press ESC in order to cancel the process. For some UV layouts it is required to use the 'Heuristic Search' option in order to obtain a decent result from the grouped packing",
                    default=False)

    grouped_pack_approach = EnumProperty(
                    items=(('0', 'Many Small Islands', '', '', 0),
                           ('1', 'Bigger Islands', '', '', 1)),
                    name="Grouped Pack Approach",
                    description="Grouped packing approach to use. The 'Many Small Islands' option should work well in most cases. You can try the 'Bigger Islands' option if the former doesn't give good results")
...
class UvPackPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(prefs.label_message)
        ...
        if not prefs.FEATURE_heuristic_search:
            ...
            row.label(feature_not_supported_msg)
        ...
        row  = box.row()
        row.prop(prefs, "grouped_pack_approach")
        ...
        # row = box.row()
        row.operator(UvOverlapCheckOperator.bl_idname)
        if not prefs.FEATURE_overlap_check:
            row.label(feature_not_supported_msg)

        col.operator(UvMeasureAreaOperator.bl_idname)
        ...
        if prefs.FEATURE_demo:
            op_text = UvValidateOperator.bl_label + " (DEMO)"
        else:
            op_text = UvValidateOperator.bl_label

        row.operator(UvValidateOperator.bl_idname, text=op_text)
        ...
        if prefs.FEATURE_demo:
            op_text = UvPackOperator.bl_label + " (DEMO)"
        else:
            op_text = UvPackOperator.bl_label

        row.operator(UvPackOperator.bl_idname, text=op_text)
...
def get_packer_version():
    packer_args = [get_packer_path(), '-o', str(UvPackerOpcode.REPORT_VERSION)]
...
def register():
    ...
    bpy.utils.register_class(UvPackOperator)
    bpy.utils.register_class(UvMeasureAreaOperator)
    bpy.utils.register_class(UvOverlapCheckOperator)
    bpy.utils.register_class(UvValidateOperator)
    bpy.utils.register_class(UvPackPanel)
    ...
    try:
        ...
        version_array = get_packer_version_array()
        ...
        prefs.label_message = 'Packer version: {}.{} ({})'.format(version_major, version_minor, version_long_name)
...
def unregister():
    bpy.utils.unregister_class(UvPackPanel)
    bpy.utils.unregister_class(UvValidateOperator)
    bpy.utils.unregister_class(UvOverlapCheckOperator)
    bpy.utils.unregister_class(UvMeasureAreaOperator)
    bpy.utils.unregister_class(UvPackOperator)
