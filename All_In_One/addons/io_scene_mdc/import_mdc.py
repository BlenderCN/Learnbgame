...
# Module: import_mdc.py
# Description: MDC import interface.
...
class MDCImport:
    ...
    def run(self, importOptions):

        mdcFile = MDCFile.read(importOptions)

        isVerified, errMsg = Verify.mdcFile(mdcFile)
        if isVerified == False:
            return errMsg

        blenderScene = Converter.mdcFileToBlenderScene(mdcFile)

        isVerified, errMsg = Verify.blenderScene(blenderScene)
        if isVerified == False:
            return errMsg

        blenderScene.write(importOptions)
