...
UMLIST = { 
            '04e57591212580' : 'vaas77.g',
            '04fe9291212580' : 'car.g',
            #'04059c91212581' : 'arm70-25.g',
            '04059c91212581' : 'test.g',
            '04987c91212580' : 'arm85-30.g',
            '049d1c7a3c2280' : 'arm65-15.g',
            '0407a791212581' : 'arm65-20.g'

         }

def read_rfid():
    rfidpath="./lib-touchatag-1.0/"
    print("* Reading RFID...")
    #bpy.ops.object.dialog_operator('INVOKE_DEFAULT') 
    p = sub.Popen(rfidpath+"main_new_reader",stdout=sub.PIPE,stderr=sub.PIPE)
    output, errors = p.communicate()
    if output.startswith(" No tags found") or output.startswith(" No readers found"):
        print(output)
        return False
...
class Miniterm:
    ...
    def start(self):
        ...
        self.console_thread.setDaemon(1)
            ...
            while self.alive:
                ...
                if self.repr_mode == 0:
                    ...
                    if data == '\r' :
                        sys.stdout.write('\n')
                        buf = buf.replace('\n',"")
                        done = buf.startswith("Done printing file")
                        if (done):
                            self.isprinting = False
                        failed = buf.startswith("file.open failed")
                        if (failed):
                            self.isprinting = False
                        buf = ""
                    else:
    ...
    def writer(self):
        ...
        try:
            while 1:
              while self.alive and not self.isprinting:
                  tagid = read_rfid()
                  if (tagid == False):
                      continue
                  tagid = tagid.replace("\n","")
                  ...
                  if tagid in UMLIST:
                    print(UMLIST[tagid])
                    self.serial.write("M23 " + UMLIST[tagid])                    # send character
                    self.serial.write('\r\n')
                    self.serial.write("M24")                    # send character
                    self.serial.write('\r\n')
                    # Wait for output buffer to drain.
                    self.serial.flush()
                    self.isprinting = True
                    #self.serial.write("M27")                    # send character
                  else:
                    print("Tag ID not in the Array");
