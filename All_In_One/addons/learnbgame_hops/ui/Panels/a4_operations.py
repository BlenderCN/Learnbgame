...
class HOPS_PT_OperationsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if active_object is None:
            ...
            colrow = col.row(align=True)
            colrow.menu("hops.edge_wizzard_submenu", text="AUX", icon="PLUGIN")
            ...
            if any("mira_tools" in s for s in bpy.context.preferences.addons.keys()):
                col.separator()
                colrow = col.row(align=True)
                colrow.menu("hops.mira_submenu", text="Mira (T)", icon="PLUGIN")
            else:
                col.separator()
            colrow = col.row(align=True)
            colrow.menu("hops.symetry_submenu", text="Symmetrize", icon_value=get_icon_id("Xslap"))
