...
class LibraryConnect(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.mode == "online":
            ...
            debugout(mat_lib_host)
            debugout(mat_lib_location)
        ...
        if self.mode == "online":
            ...
            if xml_header_string not in mat_lib_contents:
                ...
                debugout("Received following response from server:\n" + mat_lib_contents)
                return {'CANCELLED'}
        ...
        while i < mat_lib_categories:
            debugout ("Adding category #%d" % (i + 1))
...
def libraryCategoryUpdate(self, context):
    debugout("Updating Material Category.")
    ...
    if bpy.context.scene.mat_lib_material_category != "None0":
        ...
        current_material_number = -1
        category_type = "none"
    if context.active_object.material_slots[-1].material.name == "mat_lib_preview_material":
...
class LibraryClearCache(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        debugout('Clearing cache files....')
        for root, dirs, files in os.walk(cached_data_path):
            for name in files:
                if name[-4:].lower() == ".jpg":
                    debugout("Deleting \"" + os.path.join(root, name) + "\".")
    ...
    def execute(self, context):
        ...
        if self.open_location == "" and self.text_block == "":
            if library == "composite" and os.path.exists(os.path.join(matlibpath, mat_lib_host, "cycles", category_filename, self.filename + ".bcm")):
                ...
                return {'CANCELLED'}
            debugout (material_file_contents)
                ...
                if node_data['image'].value and '.' in node_data['image'].value:
                    if "file://" in node_data['image'].value:
                        ...
                        if ext.lower() != ".jpg" and ext.lower() != ".png":
                            node_message = ['WARNING', "The image file referenced by this image texture node is not .jpg or .png; not downloading."]
                            break
                        ...
                        if library == "composite" and os.path.exists(os.path.join(matlibpath, mat_lib_host, "cycles", "textures", image_name + ext)):
                            ...
                            node_message = ['WARNING', "The image texture, \"%s\", is not cached; cannot download in offline mode." % (image_name + ext)]
                            image_filepath = ""
                ...
                if node_data['script'].value and '.' in node_data['script'].value:
                    if "file://" in node_data['script'].value:
                        ...
                        if ext.lower() != ".osl" and ext.lower() != ".oso":
                            node_message = ['WARNING', "The OSL script file referenced by this script node is not .osl or .oso; not downloading."]
                            break
                        ...
                        if library == "composite" and os.path.exists(os.path.join(matlibpath, mat_lib_host, "cycles", "scripts", script_name + ext)):
                            ...
                            node_message = ['WARNING', "The OSL script, \"%s\", is not cached; cannot download in offline mode." % (script_name + ext)]
                            script_filepath = ""
                ...
                if node_data['group'].value and '.' in node_data['group'].value:
                    if "file://" in node_data['group'].value:
                        ...
                        if ("." + node_data['group'].value.split(".")[-1]).lower != ".bcg":
                            node_message = ['WARNING', "The nodegroup file referenced by this group node is not .bcg; not downloading."]
                            break
                        ...
                        if library == "composite" and os.path.exists(os.path.join(matlibpath, mat_lib_host, "cycles", "groups", group_name + ".bcg")):
                            ...
                            node_message = ['WARNING', "The nodegroup file, \"%s\", is not cached; cannot download in offline mode." % (group_name + ".bcg")]
                            group_filepath = ""
...
class AddLibraryGroup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.open_location == "" and self.text_block == "":
            ...
            #Check file for validitity
            if xml_header_string not in group_file_contents[0:40]:
    ...
    def execute(self, context):
        ...
        for tmp_group in bpy.data.node_groups:
            if tmp_group.name not in old_groups_list:
                ...
                node = node_tree.nodes.new(node_types['SHADER']['GROUP'])
                node.node_tree = tmp_group
...
def createLinks(links, node_tree):
    for dom_link in links:
        try:
            ...
            # if external scripts are missing we want to silently
            # skip missing sockets when linking
            # this allows a more complete node tree
            # instead of all unlinked nodes.
            # missing osl scripts should be the only cause of exceptions here
            pass
...
def fillNodeGroup(name, group_text):
    ...
    group_curves = []
    debugout('getting group curve data')
    for curve in curves:
        debugout('adding curve points')
        curve_points = []
        cdom = xml.dom.minidom.parseString(curve.toxml())
        points = cdom.getElementsByTagName("point")
        for point in points:
...
def addNodes(nodes, node_tree, group_mode = False):
    ...
    for dom_node in nodes:
        ...
        if node_type == "ATTRIBUTE":
            ...
            debugout ("OBJECT_INFO")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])

        elif node_type == "PARTICLE_INFO":
            debugout ("PARTICLE_INFO")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            ...
            debugout ("WIREFRAME")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.use_pixel_size = boolean(node_data['use_pixel_size'].value)
            node.inputs['Size'].default_value = float(node_data['size'].value)

        elif node_type == "UVMAP":
            debugout ("UVMAP")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.from_dupli = boolean(node_data['from_dupli'].value)
            node.uv_map = node_data['uv_map'].value

            #OUTPUT TYPES
        elif node_type == "OUTPUT_LAMP":
            debugout ("OUTPUT_LAMP")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])

        elif node_type == "OUTPUT_MATERIAL":
            debugout ("OUTPUT_MATERIAL")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])

        elif node_type == "OUTPUT_WORLD":
            debugout ("OUTPUT_WORLD")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            ...
            debugout ("BSDF_TRANSLUCENT")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = color(node_data['color'].value)

        elif node_type == "BSDF_TRANSPARENT":
            debugout ("BSDF_TRANSPARENT")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = color(node_data['color'].value)

        elif node_type == "BSDF_VELVET":
            debugout ("BSDF_VELVET")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = color(node_data['color'].value)
            ...
            debugout ("BSDF_TOON")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.component = node_data['component'].value
            node.inputs['Color'].default_value = color(node_data['color'].value)
            node.inputs['Size'].default_value = float(node_data['size'].value)
            node.inputs['Smooth'].default_value = float(node_data['smooth'].value)

        elif node_type == "VOLUME_ABSORPTION":
            debugout ("VOLUME_ABSORPTION")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = color(node_data['color'].value)
            node.inputs['Density'].default_value = float(node_data['density'].value)

        elif node_type == "VOLUME_SCATTER":
            debugout ("VOLUME_SCATTER")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = color(node_data['color'].value)
            node.inputs['Density'].default_value = float(node_data['density'].value)
            node.inputs['Anisotropy'].default_value = float(node_data['anisotropy'].value)
            ...
            debugout ("SUBSURFACE_SCATTERING")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.falloff = node_data['falloff'].value
            node.inputs['Color'].default_value = color(node_data['color'].value)
            node.inputs['Scale'].default_value = float(node_data['scale'].value)
            node.inputs['Radius'].default_value = vector(node_data['radius'].value)
            node.inputs['Sharpness'].default_value = float(node_data['sharpness'].value)
            node.inputs['Texture Blur'].default_value = float(node_data['textureBlur'].value)
            ...
            node.inputs['RoughnessU'].default_value = float(node_data['roughnessu'].value)
            node.inputs['RoughnessV'].default_value = float(node_data['roughnessv'].value)
            ...
            # addon < 0.7 added these two as height and width
            # this now conflicts with the node locations
            # if width exists and not brick_width then we have an old export
            # so we treat width and height the old way
            if 'width' in node_data and 'brick_width' not in node_data:
                ...
                # set to defaults for node properties
                node_data['width'].value = 150
                node_data['height'].value = 100
            else:
                node.inputs['Brick Width'].default_value = float(node_data['brick_width'].value)
                node.inputs['Row Height'].default_value = float(node_data['row_height'].value)

        elif node_type == "TEX_CHECKER":
            debugout ("TEX_CHECKER")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color1'].default_value = color(node_data['color1'].value)
            node.inputs['Color2'].default_value = color(node_data['color2'].value)
            node.inputs['Scale'].default_value = float(node_data['scale'].value)
            ...
            debugout ("TEX_POINTDENSITY")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.point_source = node_data['point_source'].value
            try:
                if 'object' in node_data:
                    node.object = bpy.data.objects[node_data['object'].value]
                    if 'particle_system' in node_data:
                        node.particle_system = node.object.particle_systems[node_data['particle_system'].value]
            except:
                # let's fail quietly if the scene to import to
                # doesn't have matching object and particle names
                pass
            node.space = node_data['space'].value
            node.radius = float(node_data['radius'].value)
            node.interpolation = node_data['interpolation'].value
            node.resolution = int(node_data['resolution'].value)
            node.color_source = node_data['color_source'].value

            #COLOR TYPES
        elif node_type == "BRIGHTCONTRAST":
            debugout ("BRIGHTCONTRAST")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = color(node_data['color'].value)
            ...
            debugout ("VECT_TRANSFORM")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.vector_type = node_data['vector_type'].value
            node.convert_from = node_data['convert_from'].value
            node.convert_to = node_data['convert_to'].value
            node.inputs['Vector'].default_value = vector(node_data['vector'].value)
        ...
        elif node_type == "COMBHSV":
            debugout ("COMBHSV")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['H'].default_value = float(node_data['hue'].value)
            node.inputs['S'].default_value = float(node_data['saturation'].value)
            node.inputs['V'].default_value = float(node_data['value'].value)

        elif node_type == "COMBXYZ":
            debugout ("COMBXYZ")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['X'].default_value = float(node_data['x'].value)
            node.inputs['Y'].default_value = float(node_data['y'].value)
            node.inputs['Z'].default_value = float(node_data['z'].value)
        ...
        elif node_type == "SEPHSV":
            debugout ("SEPHSV")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Color'].default_value = hsvstr(node_data['color'].value)

        elif node_type == "SEPXYZ":
            debugout ("SEPXYZ")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Vector'].default_value = vector(node_data['vector'].value)
        ...
        elif node_type == "BLACKBODY":
            debugout ("BLACKBODY")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Temperature'].default_value = float(node_data['temperature'].value)

        elif node_type == "WAVELENGTH":
            debugout ("WAVELENGTH")
            node = node_tree.nodes.new(node_types['SHADER'][node_type])
            node.inputs['Wavelength'].default_value = float(node_data['wavelength'].value)
            ...
            if 'group' in node_data and "." in node_data['group'].value:
                ...
                if "file://" in node_data['group'].value:
                    ...
                    if node_data['group'].value[-4:].lower() != ".bcg":
                        node_message = ['WARNING', "The node group file referenced by this group node is not .bcg."]
                        return
                    ...
                    if node_data['group'].value[-4:].lower() != ".bcg":
                        node_message = ['WARNING', "The node group file referenced by this group node is not .bcg; not downloading."]
                        return
                    ...
                    if library == "composite" and os.path.exists(os.path.join(matlibpath, mat_lib_host, "cycles", "groups", group_name + ".bcg")):
                        ...
                        node_message = ['WARNING', "The node group file, \"%s\", is not cached; cannot download in offline mode." % (group_name + ".bcg")]
            ...
            node_message = ['WARNING', """The material file contains the node type \"%s\", which is not known.
The material file may contain an error, or you may need to check for updates to this add-on.""" % node_type]
            return

        #Set node sizes
        if 'node_width' in node_data:
            node.width = int(node_data['node_width'].value)
        if 'width_hidden' in node_data and hasattr(node, 'width_hidden'):
            node.width_hidden = int(node_data['width_hidden'].value)
        if 'node_height' in node_data:
            node.height = int(node_data['node_height'].value)
...
def boolean(string):
    if string == "True":
        ...
        debugout('Error converting string to a boolean')
        return
    return boolean
...
def color(string):
    if "rgba" in string:
        ...
        debugout('Error converting string to a color')
        return
    return color
...
def hsvstr(string):
    if "hsv" in string:
        ...
        debugout('Error converting string to hsv')
        return
    return hsv
...
def vector(string):
    ...
    if "Vector" in string:
        ...
        debugout('Error converting string to a vector')
        return
    return vector
    ...
    def execute(self, context):
        ...
        while j < loop_length:
            ...
            if self.save_location == "":
                ...
                debugout(context.scene.mat_lib_bcm_save_location + filename + ".bcm")
...
def getNodeData(node, group_mode = False):
    ...
    if node_type == "GROUP":
        ...
        debugout("WIREFRAME")
        text += (" use_pixel_size=\"%s\"" % node.use_pixel_size)
        text += (" size=\"%s\"" % smallFloat(I['Size'].default_value))

    elif node_type == "UVMAP":
        debugout("UVMAP")
        text += (" from_dupli=\"%s\"" % node.from_dupli)
        ...
        debugout("BSDF_TOON")
        text += (" component=\"%s\"" % node.component)
        text += (" color=\"%s\"" % rgba(I['Color'].default_value))
        text += (" size=\"%s\"" % smallFloat(I['Size'].default_value))
        ...
        debugout("VOLUME_ABSORPTION")
        text += (" color=\"%s\"" % rgba(I['Color'].default_value))
        text += (" density=\"%s\"" % smallFloat(I['Density'].default_value))
        ...
        debugout("SUBSURFACE_SCATTERING")
        text += (" falloff=\"%s\"" % node.falloff)
        text += (" color=\"%s\"" % rgba(I['Color'].default_value))
        text += (" scale=\"%s\"" % smallFloat(I['Scale'].default_value))
        text += (" radius=\"%s\"" % smallVector(I['Radius'].default_value))
        text += (" sharpness=\"%s\"" % smallFloat(I['Sharpness'].default_value))
        text += (" textureBlur=\"%s\"" % smallFloat(I['Texture Blur'].default_value))
        ...
        text += (" roughnessu=\"%s\"" % smallFloat(I['RoughnessU'].default_value))
        text += (" roughnessv=\"%s\"" % smallFloat(I['RoughnessV'].default_value))
        ...
        text += (" brick_width=\"%s\"" % smallFloat(I['Brick Width'].default_value))
        text += (" row_height=\"%s\"" % smallFloat(I['Row Height'].default_value))
        ...
        debugout("TEX_POINTDENSITY")
        text += (" point_source=\"%s\"" % node.point_source)
        if node.object:
            text += (" object=\"%s\"" % node.object.name)
            if node.particle_system:
                text += (" particle_system=\"%s\"" % node.particle_system.name)
        text += (" space=\"%s\"" % node.space)
        text += (" radius=\"%s\"" % smallFloat(node.radius))
        text += (" interpolation=\"%s\"" % node.interpolation)
        text += (" resolution=\"%s\"" % str(node.resolution))
        text += (" color_source=\"%s\"" % node.color_source)
        ...
        debugout("VECT_TRANSFORM")
        text += (" vector_type=\"%s\"" % node.vector_type)
        text += (" convert_from=\"%s\"" % node.convert_from)
        text += (" convert_to=\"%s\"" % node.convert_to)
        text += (" vector=\"%s\"" % smallVector(I['Vector'].default_value))
        ...
        debugout("COMBHSV")
        text += (" hue=\"%s\"" % smallFloat(I['H'].default_value))
        text += (" saturation=\"%s\"" % smallFloat(I['S'].default_value))
        text += (" value=\"%s\"" % smallFloat(I['V'].default_value))

    elif node_type == "COMBXYZ":
        debugout("COMBXYZ")
        text += (" x=\"%s\"" % smallFloat(I['X'].default_value))
        text += (" y=\"%s\"" % smallFloat(I['Y'].default_value))
        text += (" z=\"%s\"" % smallFloat(I['Z'].default_value))
        ...
        debugout("SEPHSV")
        text += (" color=\"%s\"" % hsv(I['Color'].default_value))
        ...
        debugout("BLACKBODY")
        text += (" temperature=\"%s\"" % smallFloat(I['Temperature'].default_value))

    elif node_type == "WAVELENGTH":
        debugout("WAVELENGTH")
        text += (" wavelength=\"%s\"" % smallFloat(I['Wavelength'].default_value))
...
def hsv(color):
    hue = smallFloat(color[0])
    saturation = smallFloat(color[1])
    value = smallFloat(color[2])
    alpha = smallFloat(color[3])
...
def getLocation(node):
    locationData = (" node_height=\"%s\"" % int(node.height))
    locationData += (" node_width=\"%s\"" % int(node.width))
...
def register():
    ...
    bpy.types.Scene.mat_lib_material_category = bpy.props.EnumProperty(
        items = category_enum_items, name = "",
        description = "Choose a category", update=libraryCategoryUpdate)
