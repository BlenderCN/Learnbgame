...
description : 
This is the core of the program "Simblend". The called class is SimionToAnimation upon startup

Sources that were used during the programm creation
...
'''	
This is the mainsourcecode of Simblend. 
Overview of calls:
1. Create a list of Ions from the import file specified in the class call using the FileIo.py
...
def main(file):
	...
	if bpy.context.scene.animation_type == 'particle_id':
		for item in windowed_list:
			...
			ob.select = True
			bpy.context.scene.objects.active = ob
			ob.keyframe_insert(data_path="location",frame = float(windowed_list[item].GetToF(0)))
