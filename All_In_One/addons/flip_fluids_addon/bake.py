...
def __initialize_fluid_simulation_settings(fluidsim, data):
    ...
    if is_surface_tension_enabled:
        ...
        minscale, maxscale = 1.0, 2.0
        stability_pct = __get_parameter_data(world.surface_tension_stability, frameno) / 100.0
        surface_tension_scale = minscale + stability_pct * (maxscale - minscale)
        fluidsim.surface_tension_particle_scale = surface_tension_scale
...
def __update_animatable_inflow_properties(data, frameno):
    ...
    for idx, inflow in enumerate(inflow_objects):
        ...
        is_constrained = __get_parameter_data(data.constrain_fluid_velocity, frameno)
...
def __update_animatable_obstacle_properties(data, frameno):
    ...
    for idx, mesh_object in enumerate(obstacle_objects):
        ...
        mesh_object.whitewater_influence = __get_parameter_data(data.whitewater_influence, frameno)
...
def __update_animatable_domain_properties(fluidsim, data, frameno):
    ...
    if is_surface_tension_enabled:
        ...
        stability_pct = __get_parameter_data(world.surface_tension_stability, frameno) / 100.0
        surface_tension_scale = minscale + stability_pct * (maxscale - minscale)
        __set_property(fluidsim, 'surface_tension_particle_scale', surface_tension_scale)
