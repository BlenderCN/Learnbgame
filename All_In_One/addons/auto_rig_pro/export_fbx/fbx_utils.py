	...
	def get_matrix_local(self):
		if self._tag == 'OB':
			...
			if bpy.context.scene.arp_fix_fbx_matrix:
				par_mat = self._ref.pose.bones[par.name].matrix if par else Matrix()
				t_mat_par = Matrix.Translation(par_mat.to_translation())
				r_mat_par = par_mat.to_quaternion().to_matrix().to_4x4()			
				s_mat_par = extract_scale_matrix(par_mat.to_3x3()).to_4x4()
				par_mat_recomposed = t_mat_par @ r_mat_par @ s_mat_par
				par_mat_inv = par_mat_recomposed.inverted_safe()
