...
# check if any of the images have to be resized
...
class BrushPopup(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        row.prop(ups, "size", text="Radius", slider=True)
        row.prop(ups, "use_pressure_size", toggle=True, text="")
        # strength buttons
        row = col.row(align=True)
...
class CycleChannels(bpy.types.Operator):
    '''Cycle through different channel modes in the UV/Image editor'''
    bl_idname = "paint.cycle_channels"
    bl_label = "Cycle channels"
    ...
    def invoke(self, context, event):
        channel = context.space_data.draw_channels
        # test which channel draw modes are available
        options = ['COLOR', 'COLOR_ALPHA', 'ALPHA', 'Z_BUFFER']
        possible = []
        for o in options:
            try:
                context.space_data.draw_channels = o
                possible.append(o)
            except:
                pass
        # change to next available channel draw mode
        new = possible.index(channel) + 1
        if new >= len(possible):
            new = 0
        context.space_data.draw_channels = possible[new]
...
class DefaultMaterial(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        ts.texture = tex
        # diffuse, set image if possible
        try:
            image = ob.data.uv_textures[0].data[0].image
        except:
            image = False
        if not image:
...
class DisplayToolMode(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        if context.window_manager.tpp.toolmode_enabled == 0:
            ...
            self._handle = bpy.types.SpaceView3D.draw_handler_add(toolmode_draw_callback,
...
class RemoveImage(bpy.types.Operator):
    '''Remove currently displayed image'''
    bl_idname = "wm.remove_image"
...
class RepeatWrap(bpy.types.Operator):
    '''Toggles both coordinate repetition and wrapping of the paint stroke'''
    bl_idname = "paint.repeat_wrap"
    bl_label = "Repeat + Wrap"
    ...
    def invoke(self, context, event):
        ...
        repeat = space.show_repeat
        brush = context.tool_settings.image_paint.brush
        wrap = brush.use_wrap
        
        if not (repeat and wrap):
            space.show_repeat = True
            brush.use_wrap = True
        else:
            space.show_repeat = False
            brush.use_wrap = False
    ...
    def invoke(self, context, event):
        ...
        for img in images:
            try:
                ...
                result[1] += 1
        message = "Saved %s image" % result[0]
        if result[0] > 1:
            message += "s"
...
class SelectLinked(bpy.types.Operator):
    '''Select all vertices linked to the active mesh'''
    bl_idname = "mesh.select_linked_tpp"
    bl_label = "TPP: select linked"
    
    @classmethod
    def poll(cls, context):
        return(bpy.ops.mesh.select_linked.poll())
    
    def invoke(self, context, event):
        props = context.window_manager.tpp
        
        if time.time() - props.select_linked < 2:
            bpy.ops.ed.undo()
            bpy.ops.mesh.select_linked(limit=True)
            props.select_linked = 0.0
        else:
            bpy.ops.mesh.select_linked(limit=False)
            props.select_linked = float(time.time())
        
        return {'FINISHED'}


class ShadelessViewport(bpy.types.Operator):
    '''Toggle shadeless shading in viewport'''
    bl_idname = "wm.shadeless_viewport"
    bl_label = "Shadeless shading"
    ...
    def invoke(self, context, event):
        props = context.window_manager.tpp
        status = props.shadeless_viewport

        # turn shadeless shading on
        if not status:
            props.shadeless_old_shade = context.space_data.viewport_shade
            props.shadeless_old_solid = context.space_data.show_textured_solid
            context.space_data.viewport_shade = 'SOLID'
            context.space_data.show_textured_solid = True
            for mat in bpy.data.materials:
                mat['TPP_shadeless_status'] = mat.use_shadeless
                mat.use_shadeless = True 
        # turn shadeless shading off
        else:
            context.space_data.viewport_shade = props.shadeless_old_shade
            context.space_data.show_textured_solid = props.shadeless_old_solid
            for mat in bpy.data.materials:
                if 'TPP_shadeless_status' in mat:
                    mat.use_shadeless = mat['TPP_shadeless_status']
        
        # toggle shadeless shading status
        context.window_manager.tpp.shadeless_viewport = not status
        
        return {'FINISHED'}


class SnapRotate(bpy.types.Operator):
    '''Set snapping to "increment", while in rotate mode'''
    bl_idname = "transform.snap_rotate"
    bl_label = "Rotate"
    
    def invoke(self, context, event):
        # different areas have different snap elements
        if context.area.type == 'IMAGE_EDITOR':
            se = "snap_uv_element"
        else:
            se = "snap_element"
        
        old_snap_element = getattr(context.tool_settings, se)
        setattr(context.tool_settings, se, 'INCREMENT')
        bpy.ops.transform.rotate('INVOKE_DEFAULT')
        setattr(context.tool_settings, se, old_snap_element)
...
class StraightLine(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'LEFTMOUSE' and event.value == 'RELEASE':
            if StraightLine._handle is not None:
                bpy.types.SpaceImageEditor.draw_handler_remove(StraightLine._handle, 'WINDOW')
                StraightLine._handle = None
            x0, y0, x1, y1 = context.window_manager["straight_line"]
    ...
    def invoke(self, context, event):
        ...
        StraightLine._handle = bpy.types.SpaceImageEditor.draw_handler_add(draw_callback, (self, context),
            'WINDOW', 'POST_PIXEL')
        context.window_manager.modal_handler_add(self)
...
class SyncSelection(bpy.types.Operator):
    ...
    def remove_handles(self, context):
        if SyncSelection._handle1 is not None:
            bpy.types.SpaceView3D.draw_handler_remove(SyncSelection._handle1, 'WINDOW')
            SyncSelection._handle1 = None
        if SyncSelection._handle2 is not None:
            bpy.types.SpaceView3D.draw_handler_remove(SyncSelection._handle2, 'WINDOW')
            SyncSelection._handle2 = None
        if SyncSelection._handle3 is not None:
            bpy.types.SpaceImageEditor.draw_handler_remove(SyncSelection._handle3, 'WINDOW')
            SyncSelection._handle3 = None
        self.area = None
        self.region = None
                ...
                if area.type == 'VIEW_3D':
                    ...
                    for region in area.regions:
                        if region.type == 'WINDOW':
                            ...
                            selected = [mesh.loops[i].vertex_index for i, uvloop in enumerate(uvl.data) if uvloop.select]
                            ...
                            self.overlay_faces = [face.index for face in mesh.tessfaces if face.select]
                            
                            # overlay positions for image editor
                            self.position2_vertices = []
                            for i, loop in enumerate(mesh.loops):
                                if mesh.vertices[loop.vertex_index].select:
                                    self.position2_vertices.append(uvl.data[i].uv[:])
                            ...
                            # 3d view callbacks
                            SyncSelection._handle1 = bpy.types.SpaceView3D.draw_handler_add(sync_calc_callback,
                                (self, context, area, region), 'WINDOW', 'POST_VIEW')
                            SyncSelection._handle2 = bpy.types.SpaceView3D.draw_handler_add(sync_draw_callback,
                                (self, context), 'WINDOW', 'POST_PIXEL')
                            
                            # image editor callback
                            SyncSelection._handle3 = bpy.types.SpaceImageEditor.draw_handler_add(sync_draw_callback2,
...
class TextureFilepath(bpy.types.Operator):
    '''Change the base of all image paths'''
    bl_idname = "image.change_basepaths"
    bl_label = "Change"
    ...
    def invoke(self, context, event):
        ...
        if props.texture_filepath_rel:
            filepath = bpy.path.relpath(props.texture_filepath)
        else:
            filepath = props.texture_filepath
        
        for img in bpy.data.images:
            new_path = os.path.join(filepath, bpy.path.basename(img.filepath))
            if img.filepath != new_path:
                img.filepath = new_path
                changed += 1
        
        self.report({'INFO'}, "Changed the filepath of %s images"%changed)
...
class ToggleTexture(bpy.types.Operator):
    '''Enable or disable the brush texture'''
    bl_idname = "paint.toggle_texture"
    ...
    def invoke(self, context, event):
        ...
        if not brush.texture:
            if props.toggle_texture:
                if props.toggle_texture in bpy.data.textures:
                    brush.texture = bpy.data.textures[props.toggle_texture]
                else:
                    self.report({'ERROR'}, "Texture has been removed or renamed")
            else:
                self.report({'ERROR'}, "Found no original texture")
        else:
            props.toggle_texture = brush.texture.name
            brush.texture = None
...
# bulk change texture filepaths
class VIEW3D_PT_texture_filepaths(bpy.types.Panel):
    bl_label = "Image filepaths"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'TOOLS'
    ...
    def draw(self, context):
        ...
        layout.prop(props, "texture_filepath")
        layout.prop(props, "texture_filepath_rel")
        layout.operator("image.change_basepaths")
...
class TexturePaintPlusProps(bpy.types.PropertyGroup):
    ...
    consolidate_width = bpy.props.IntProperty(name = "X",
        ...
        min = 1,
        default = 1024)
    select_linked = bpy.props.FloatProperty(name = "Limit seams",
        description = "internal use",
        default = 0.0)
    shadeless_viewport = bpy.props.BoolProperty(name = "Shadeless",
        description = "internal use",
        default = False)
    shadeless_old_shade = bpy.props.EnumProperty(name = "Old shading",
        items = (("BOUNDBOX", "Bounding box", ""),
            ("WIREFRAME", "Wireframe", ""),
            ("SOLID", "Solid", ""),
            ("TEXTURED", "Textured", ""),
            ("MATERIAL", "Material", ""),
        ...
        description = "internal use",
        default = 'WIREFRAME')
    shadeless_old_solid = bpy.props.BoolProperty(name = "Old textured solid",
        description = "internal use",
        default = False)
    sync_enabled = bpy.props.IntProperty(name = "Enabled",
        description = "internal use",
        default = 0)
    texture_filepath = bpy.props.StringProperty(name = "Filepath",
        description = "New basepath for all texture images",
        default = "//",
        subtype = 'DIR_PATH')
    texture_filepath_rel = bpy.props.BoolProperty(name = "Relative",
        description = "use paths relative to the current blend-file",
        default = False)
    toggle_texture = bpy.props.StringProperty(name = "Old texture",
        description = "internal use",
        default = "")
...
#    CycleBlendtype,
    ...
    VIEW3D_PT_texture_filepaths,
    TexturePaintPlusProps]


def repeat_wrap_func(self, context):
    if context.space_data.show_paint:
        self.layout.operator("paint.repeat_wrap")
...
def register():
    ...
    kmi = km.keymap_items.new("paint.toggle_alpha_mode", 'O', 'PRESS')
#    kmi = km.keymap_items.new("paint.cycle_blendtype", 'O', 'PRESS',
#        ctrl=True)
    ...
    kmi = km.keymap_items.new("paint.display_tool_mode", 'G', 'PRESS',
        ctrl=True)
    ...
    kmi = km.keymap_items.new("paint.change_selection", 'NUMPAD_PLUS', 'PRESS',
        ctrl=True)
    kmi = km.keymap_items.new("paint.brush_popup", 'C', 'PRESS')
    kmi = km.keymap_items.new("wm.context_toggle", 'W', 'PRESS')
    ...
    kmi = km.keymap_items.new("wm.context_set_enum", 'I', 'PRESS')
    ...
    kmi = km.keymap_items.new("paint.save_image", 'S', 'PRESS',
        alt=True)
    kmi = km.keymap_items.new("paint.toggle_texture", 'T', 'PRESS',
        ctrl=True)
    kmi = km.keymap_items.new("paint.cycle_channels", 'T', 'PRESS',
        shift=True)
    ...
    for kmi in km.keymap_items:
        if kmi.type == 'R' and not kmi.any and not kmi.shift and not kmi.ctrl and not kmi.alt and not kmi.oskey:
            kmi.active = False
    
    # add 3D View keymap entry
    km = bpy.context.window_manager.keyconfigs.default.keymaps['3D View']
    kmi = km.keymap_items.new("object.add_default_image", 'Q', 'PRESS')
    kmi = km.keymap_items.new("transform.snap_rotate", 'R', 'PRESS')
    kmi = km.keymap_items.new("wm.context_menu_enum", 'V', 'PRESS',
        shift=True)
    kmi.properties.data_path = "space_data.pivot_point"
    kmi = km.keymap_items.new("object.default_material", 'X', 'PRESS',
        alt=True, ctrl=True)
    kmi = km.keymap_items.new("wm.shadeless_viewport", 'Z', 'PRESS',
        shift=True)
    
#    for kmi in km.keymap_items:
#        if kmi.type == 'C' and not kmi.any and not kmi.alt and kmi.ctrl and not kmi.shift and not kmi.oskey:
#            kmi.active = False
...
#        ctrl=True)
#    kmi = km.keymap_items.new("mesh.select_linked_tpp", 'C', 'PRESS',
#        ctrl=True)
    
    # deactivate to prevent clashing
    if 'UV Editor' in bpy.context.window_manager.keyconfigs.default.keymaps:
    ...
    kmi = km.keymap_items.new("wm.save_images", 'S','PRESS',
        alt=True, ctrl=True)
    kmi = km.keymap_items.new("wm.remove_image", 'X', 'PRESS',
        ctrl=True)
    ...
    # add to panels
    bpy.types.IMAGE_PT_view_properties.append(repeat_wrap_func)
    bpy.types.IMAGE_PT_paint_stroke.append(repeat_wrap_func)


def unregister():
    # remove from panels
    bpy.types.IMAGE_PT_view_properties.remove(repeat_wrap_func)
    bpy.types.IMAGE_PT_paint_stroke.remove(repeat_wrap_func)
    ...
    for kmi in km.keymap_items:
        ...
        "paint.toggle_add_multiply", "paint.grid_texture", "paint.display_tool_mode", "paint.reload_image", "paint.save_image", "paint.toggle_texture", "paint.cycle_channels"]:
    ...
    km = bpy.context.window_manager.keyconfigs.default.keymaps['3D View']
    for kmi in km.keymap_items:
        if kmi.idname in ["object.add_default_image", "object.default_material", "wm.shadeless_viewport", "transform.snap_rotate"]:
            km.keymap_items.remove(kmi)
        elif kmi.idname == "wm.context_menu_enum":
            if getattr(kmi.properties, "data_path", False) in ["space_data.pivot_point"]:
                km.keymap_items.remove(kmi)
    
    # remove Mesh keymap entry
    km = bpy.context.window_manager.keyconfigs.default.keymaps['Mesh']
    for kmi in km.keymap_items:
        if kmi.idname in ["paint.cycle_blendtype", "mesh.select_linked_tpp"]:
            km.keymap_items.remove(kmi)
    ...
    if 'UV Editor' in bpy.context.window_manager.keyconfigs.default.keymaps:
        km = bpy.context.window_manager.keyconfigs.default.keymaps['UV Editor']
        for kmi in km.keymap_items:
            if kmi.idname in ["transform.snap_type", "uv.sync_selection", "uv.toggle_uv_select_sync", "paint.toggle_image_paint", "paint.cycle_channels", "transform.snap_rotate"]:
                km.keymap_items.remove(kmi)

    # remove Window keymap entry
    km = bpy.context.window_manager.keyconfigs.default.keymaps['Window']
    for kmi in km.keymap_items:
        if kmi.idname in ["wm.mass_link_append", "paint.reload_images", "wm.save_images", "wm.remove_image"]:
            km.keymap_items.remove(kmi)
    ...
    for kmi in km.keymap_items:
        if kmi.type == 'R' and not kmi.any and not kmi.shift and not kmi.ctrl and not kmi.alt and not kmi.oskey:
            kmi.active = True
        elif kmi.type in right_mouse and kmi.alt and not kmi.ctrl and not kmi.shift:
            if kmi.active:
                # remap
                kmi.alt = False
            else:
    ...
    # reactive original items
    km = bpy.context.window_manager.keyconfigs.default.keymaps['Mesh']
    for kmi in km.keymap_items:
        if kmi.type == 'C' and not kmi.any and not kmi.shift and kmi.ctrl and not kmi.alt and not kmi.oskey:
            kmi.active = True
    if 'UV Editor' in bpy.context.window_manager.keyconfigs.default.keymaps:
        km = bpy.context.window_manager.keyconfigs.default.keymaps['UV Editor']
        for kmi in km.keymap_items:
