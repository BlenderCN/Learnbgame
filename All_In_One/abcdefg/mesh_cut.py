...
def outside_loop_2d(loop, neg = False):
    '''
    args:
    loop: list of 
...
def nearest_point_to_path(test_vert, vert_path, cyclic = False):
    '''
    find the closest point on a path to a test point
    ...
    return index in list
    '''    
    best_d = (vert_path[0] - test_vert).length
    v_best = vert_path[0]
    for i in range(0,len(vert_path) + 1*cyclic - 1):
        n = int(math.fmod(i+1, len(vert_path)))
        v1 = vert_path[n]
        v2 = vert_path[i]
        verif = intersect_point_line(test_vert, v1.to_3d(),v2.to_3d())
        
        if verif[1] > 0 and verif[1] <= 1:
            d = (test_vert - verif[0]).length 
            if d < best_d:
                v_best = verif[0] 
    return v_best
