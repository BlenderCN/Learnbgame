...
def bevel_method(obj):
    for mod in obj.modifiers:
        if mod.type == "BEVEL":
            mod.limit_method = get_preferences().workflow_mode


def sort(context, obj):
    if get_preferences().sort_modifiers:

        mod_types = []
        if get_preferences().sort_bevel and not get_preferences().sort_bevel_last: mod_types.append('BEVEL')
        if get_preferences().sort_array: mod_types.append('ARRAY')
        if get_preferences().sort_mirror: mod_types.append('MIRROR')
        if get_preferences().sort_weighted_normal: mod_types.append('WEIGHTED_NORMAL')
        if get_preferences().sort_simple_deform: mod_types.append('SIMPLE_DEFORM')
        if get_preferences().sort_triangulate: mod_types.append('TRIANGULATE')
        ...
        sort_bevel = get_preferences().sort_bevel
        sort_last_bevel = get_preferences().sort_bevel_last
...
def apply(obj=None, mod=None, visible=False, modifiers=[], exclude=[], type=['NONE'], kitops=False):
                ...
                if mod in modifiers:
                    ...
                    if visible:
                        if mod.show_viewport:
                            if type == 'NONE' or mod.type in type:
                                mods.append(mod)
                                continue

                    elif type == 'NONE' or mod.type in type:
                        mods.append(mod)
                        continue
    ...
    if mods:
        ...
        duplicate = object.duplicate(obj, link=True)
...
class duplicate:
    ...
    @staticmethod
    def vertex_weight_mask(self, layout, mod):
