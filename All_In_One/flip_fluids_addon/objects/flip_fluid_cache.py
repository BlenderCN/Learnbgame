...
class FlipFluidLoadedMeshData(bpy.types.PropertyGroup):
    @classmethod
    def register(cls):
        cls.mesh_prefix = StringProperty(default="mesh_prefix")
        cls.enable_motion_blur = BoolProperty(default=False)
        cls.motion_blur_scale = FloatProperty(default=-1.0)
        cls.wwp_import_percentage = IntProperty(default=0)
        cls.duplivert_scale = FloatProperty(default=1.0)
        cls.is_rendering = BoolProperty(default=True)
        cls.frame = IntProperty(default=-1)
...
class FlipFluidMeshCache(bpy.types.PropertyGroup):
    ...
    def register(cls):
        ...
        cls.loaded_frame_data = PointerProperty(type=FlipFluidLoadedMeshData)
        cls.loaded_duplivert_frame_data = PointerProperty(type=FlipFluidLoadedMeshData)
    ...
    def update_transforms(self):
        ...
        eps = 1e-4
        simx = domain_bounds.xdim / self.bounds.dx
        simy = domain_bounds.ydim / self.bounds.dx
        simz = domain_bounds.zdim / self.bounds.dx
        if abs(simx - math.floor(simx)) < eps:
            simx = math.floor(simx)
        if abs(simy - math.floor(simy)) < eps:
            simy = math.floor(simy)
        if abs(simz - math.floor(simz)) < eps:
            simz = math.floor(simz)

        scalex = (math.ceil(simx) * self.bounds.dx) / self.bounds.width
        scaley = (math.ceil(simy) * self.bounds.dx) / self.bounds.height
        scalez = (math.ceil(simz) * self.bounds.dx) / self.bounds.depth
