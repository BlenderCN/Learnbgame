...
def join_objects():
    ...
    for o in bpy.context.scene.objects:
        if o.name.startswith(obj_prefix):
            o.select_set(True)
            _hy_anon_var_2 = o.select_get()
        else:
            _hy_anon_var_2 = None
    bpy.context.view_layer.objects.active = bpy.context.scene.objects['SpaceStation_Beam']
...
def part_bevelbox(seed, config, z):
    bpy.ops.mesh.primitive_cube_add(size=rng_float(seed, 1, config['bevelbox_min'], config['bevelbox_max']), rotation=[0, 0, rng_float(seed, 2, 0.0, 3.1415)], location=[0, 0, z])
    rename()
