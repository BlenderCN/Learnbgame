...
def sort(obj, flip=False):
    if addon.preference().sort_modifiers:
        ...
        if addon.preference().sort_simple_deform: mod_types.append('SIMPLE_DEFORM')
        ...
        for mod in new.modifiers:
            if mod.type == 'SOLIDIFY' or (not flip and mod.type not in mod_types) or (flip and mod.type in mod_types):
