...
class TheGrove6(Operator, AddObjectHelper):
    ...
    bl_idname = "mesh.the_grove_6"
    bl_label = "The Grove 6"
    ...
    def invoke(self, context, event):
        ...
        if context.scene.unit_settings.system == 'NONE':
            if context.user_preferences.addons[__package__].preferences.set_scene_units:
        ...
        self.cursor_location = context.scene.cursor_location
        print("")
        print("The Grove")
        ...
        if context.user_preferences.edit.use_global_undo is False or context.user_preferences.edit.undo_steps < 8:
    ...
    def draw_actions(self, box):
        ...
        split_zoom_simulate = box.split(0.33, align=True)
    ...
    def draw_panel_simulate(self, layout, context):
        ...
        header_split = box.split(0.88)
        header_split.label(t("Simulate"))
        box.separator()
        ...
        if len(self.empties):
            box.label(t('growing_from_empties_info'), icon='INFO')
        ...
        if self.grow_years > 15:
            box.label(t('time_info'), icon='TIME')
            box.separator()
    ...
    def draw_info(self, context, box):
        ...
        if self.display_prune_warning:
            box.label(t('grease_pencil_info'), icon='ERROR')
                ...
                box.label(t('scale_to_twig_warning'), icon='ERROR')

        elif self.number_of_trees != len(self.trunks):
            if len(self.empties) != len(self.trunks):
                box.label(t('restart_info'), icon='ERROR')

        elif self.number_of_branches == self.number_of_trees:
            box.label(t('get_started_info'), icon='INFO')

        elif len(self.trunks) == 1:
            if context.scene.unit_settings.system == 'IMPERIAL':
                box.label(t('your_tree_is_info_feet').format(self.age, self.height * self.scale_to_twig / 0.3048,
                                                             self.number_of_branches))
            else:
                box.label(t('your_tree_is_info').format(self.age, self.height * self.scale_to_twig,
                                                        self.number_of_branches))
        else:
            if context.scene.unit_settings.system == 'IMPERIAL':
                box.label(t('your_trees_are_info_feet').format(self.age, self.height * self.scale_to_twig / 0.3048,
                                                               self.number_of_branches))
            else:
                box.label(t('your_trees_are_info').format(self.age, self.height * self.scale_to_twig,
                                                          self.number_of_branches))
    ...
    def draw_panel_preset(self, layout, context):
        ...
        header_split = box.split(0.88, align=True)
        header_split.label(t("Presets"))
        ...
        if self.advanced_ui_presets:
            ...
            if self.remove_preset:
                box.label(t('remove_preset_info').format(self.presets_menu), icon='ERROR')
                row = box.row(align=True)
                row.scale_y = 1.3
            ...
            if self.replace_preset:
                box.label(t('overwrite_preset_info').format(self.preset_name), icon='ERROR')
                row = box.row(align=True)
                row.scale_y = 1.3
    ...
    def draw_panel_repeat(self, context, layout):
        ...
        header_split = box.split(0.88)
        header_split.label(t("Repeat"))
        box.separator()

        self.draw_actions(box)

        split = box.split(0.33, align=True)
        row = split.row(align=True)
        row.scale_y = 1.7
    ...
    def draw_panel_animate(self, context, layout):
        ...
        box = layout.box()
        header_split = box.split(0.88)
        header_split.label(t("Animate"))
        box.separator()

        row = box.split(0.0, align=True)
        row.scale_y = 1.4
        ...
        if self.tree_has_animation:
            box.separator()
            box.label(t('wind_loop_info').format(self.wind_shapes, self.wind_shapes * 10), icon='INFO')
        box.separator()
    ...
    def draw_panel_twigs(self, context, layout):
        ...
        if self.display_vanishing_twig_warning:
            box.label(t('twig_vanished_info'), icon='ERROR')
        ...
        if self.twigs_menu == t('no_twigs'):
            ...
            if self.display_empty_twig_warning:
                row = box.row(align=False)
                row.scale_y = 1.4
                row.label(t('empty_twig_warning'), icon='ERROR')
    ...
    def draw_panel_build(self, context, layout):
        ...
        if self.build_type == '2' or self.build_type == '4':
            ...
            if self.number_of_branches == 1:
                row.label(t('branch_has_polygons_info').format(self.number_of_branches, self.number_of_polygons))
            else:
                row.label(t('branches_have_polygons_info').format(self.number_of_branches, self.number_of_polygons))
        ...
        if self.build_type == '4':
            ...
            wind_shapes_split = box.split(0.75, align=True)
        ...
        if self.build_type == '2' or self.build_type == '4':
            ...
            if self.build_type == "4":
                row.enabled = False
            row.label(t('label_layers') + ':')
            if self.display_vertex_colors_warning:
                box.label(t('vertex_color_limit_info'), icon='INFO')
                box.separator()
            row = box.row(align=False)
            row.enabled = False
    ...
    def draw(self, context):
        ...
        if self.display_undo_warning:
            layout.separator()
            layout.label(t('undo_warning'), icon='ERROR')
            layout.separator()

        if self.empties_presets:
            layout.separator()
            layout.label(t('preset_empty_info'), icon='INFO')
            layout.separator()
